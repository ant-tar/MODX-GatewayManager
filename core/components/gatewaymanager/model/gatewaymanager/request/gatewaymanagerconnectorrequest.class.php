 ...
 */
class gatewaymanagerConnectorRequest extends modConnectorResponse {

	function __construct(GatewayManager &$gatewaymanager, array $config = array()) {
		parent::__construct($gatewaymanager->modx, $config);
	...
	public function handle($action = '') {
		...
		if(!isset($this->modx->error)) {
			$this->loadErrorHandler();
		}
		
		$path = $this->gatewaymanager->config['processorsPath'].strtolower($action).'.php';
		$processorOutput = false;
		
		if(file_exists($path)) {
			$this->modx->lexicon->load('gatewaymanager:default');
